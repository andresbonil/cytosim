...
void Simul::report0(std::ostream &out, std::string const &arg, Glossary &opt) const
{
    ...
    if (who == "fiber")
    {
        ...
        if (what == "num")
            return reportFiberNum(out);

        throw InvalidSyntax("I only know fiber: position, end, point, moment, speckle, sample, segment, dynamic, length, distribution, tension, force, cluster, age, energy, hand, link, num");
    }
    if (who == "bead")
    {
        if (what.empty())
...
void Simul::reportFiberNum(std::ostream &out) const
{
    int id = -1;
...
void Simul::reportCoupleForce(std::ostream &out, Glossary &opt) const
{
    ...
    if (1)
    {
        out << COM << "force_distribution"
            << " (`scale` indicates the center of each bin)";
        out << LIN << ljust("scale", 2);
        for (unsigned u = 0; u <= nbin; ++u)
            out << " " << std::setw(5) << delta * (u + 0.5);
    }
